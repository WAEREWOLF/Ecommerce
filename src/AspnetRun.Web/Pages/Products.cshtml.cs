...
namespace AspnetRun.Web.Pages
{
    public class ProductsModel : PageModel
    {
        ...
        public IEnumerable<ProductViewModel> ProductListCopy { get; set; } = new List<ProductViewModel>();        
        ...
        public async Task OnGetAsync(string catId)
        {
            ProductList = await _productPageService.GetProducts(SearchTerm);
            ProductListCopy = await _productPageService.GetProducts(null);  

            if (SearchTerm=="")
                this.catId = Int32.Parse(catId);
            else this.catId = 10;
        }
